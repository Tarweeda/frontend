...
const SECTION_LABELS: Record<SectionKey, string> = {
  ...
  story: 'Our Story',
  values: 'Values',
  pillars: 'Pillars Strip',
  shop: 'Shop Section',
  catering: 'Catering Section',
  supper: 'Supper Club Section',
  hire: 'Hire Staff Section',
  hampers: 'Hampers Section',
...
const SECTION_ORDER: SectionKey[] = [
  'hero', 'story', 'values', 'pillars', 'shop', 'catering', 'supper', 'hire', 'hampers', 'contact', 'nav', 'footer',
];

export function AdminSiteContent() {
  const { data: overrides, isLoading } = useSiteContent();
    ...
    <div>
      <div className="admin-top-bar">
        <div className="admin-page-header">
          <h1>Site Content</h1>
          <p>Edit the text and images shown on the public website. Changes go live immediately after saving.</p>
        </div>
      </div>
        ...
        <div className="sc-panels">
          {SECTION_ORDER.map((key) => (
            <SectionPanel
              ...
              initial={mergeSection(key, overrides?.[key])}
            />
          ))}
        </div>
      )}
...
function mergeSection<K extends SectionKey>(key: K, override: Record<string, unknown> | undefined): SiteContent[K] {
...
interface PanelProps<K extends SectionKey> {
  sectionKey: K;
  label: string;
  initial: SiteContent[K];
}

function SectionPanel<K extends SectionKey>({ sectionKey, label, initial }: PanelProps<K>) {
  ...
  const save = async () => {
    setSaving(true);
    try {
      await api.put(`/admin/site-content/${sectionKey}`, { value });
  ...
  const reset = () => setValue(SITE_CONTENT_DEFAULTS[sectionKey]);

  return (
    <div className="sc-panel">
      <button type="button" className="sc-panel-head" onClick={() => setOpen((o) => !o)}>
        <span className="sc-panel-title">{label}</span>
        <span className={`sc-panel-chevron ${open ? 'open' : ''}`}>▾</span>
      </button>
      {open && (
        <div className="sc-panel-body">
          <SectionEditor sectionKey={sectionKey} value={value} onChange={setValue} />
          <div className="sc-panel-actions">
            <Button variant="ghost" type="button" onClick={reset}>Reset to default</Button>
            <Button variant="primary" type="button" loading={saving} onClick={save}>Save {label}</Button>
          </div>
        </div>
      )}
    </div>
...
function SectionEditor<K extends SectionKey>({ sectionKey, value, onChange }: { sectionKey: K; value: SiteContent[K]; onChange: (v: SiteContent[K]) => void }) {
  switch (sectionKey) {
